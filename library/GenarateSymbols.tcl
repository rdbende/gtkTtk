...
proc process {data} {
  ...
  foreach lib {gtk gdk xlib g} {
    foreach s [regexp -inline -all "GtkTtk_${lib}_${skip}\\\s*\\\(" $data] {
...
foreach file [lsort [glob -directory $sources -type f *]] {
  if {[string match *gtkTtk_Symbols* $file ]} continue
  set fd [open $file]
...
## Remove definitions...
foreach s {g_new0} {
  unset -nocomplain Symbols(GtkTtk_$s)
}
...
##
## Generate file gtkTtk_Symbols.cpp
##
set fc [open $sources/gtkTtk_Symbols.cpp w]
...
##
## Generate file gtkTtk_Symbols.h
##

set fd [open $sources/gtkTtk_Symbols.h w]
...
if {1} {
  foreach symbol [lsort -dictionary [array names Symbols]] {
    set gtk_s [string range $symbol 7 end]
...
proc initialise_lib {lib} {
  ...
  puts $fd "extern int\
            GtkTtk_InitialiseSymbols_${lib}(Tcl_Interp *interp, Tcl_Obj *file_name);"
  puts $fc "\nint\
            GtkTtk_InitialiseSymbols_${lib}(Tcl_Interp *interp, Tcl_Obj *file_name) \{"
  ...
  foreach s $GTKSymbols($lib) {
    puts $fc "  GtkTtk_$s = (Ptr_$s) Tcl_FindSymbol(interp, loadHandle, \"$s\");"
    puts $fc "  if (GtkTtk_$s == NULL) return TCL_ERROR;"
  }
  puts $fc "#else"
  ...
  puts $fc "  }"
  foreach s $GTKSymbols($lib) {
    puts $fc "  if (!g_module_symbol(m, \"$s\", (gpointer *) &GtkTtk_$s)) {"
  ...
  puts $fd "#define GTKTTK_LAST_SYMBOL_$lib GtkTtk_$s"
};# initialise_lib
