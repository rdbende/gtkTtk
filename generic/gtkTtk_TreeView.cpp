...
{
    if (!GtkTtk_GtkInitialised()) NO_GTK_STYLE_ENGINE;
    NULL_PROXY_WIDGET(GtkTtk_QWidget_Widget);
    Tcl_MutexLock(&gtkTtkMutex);
    QPixmap      pixmap(b.width, b.height);
    QPainter     painter(&pixmap);
    GTKTTK_PAINT_BACKGROUND_BASE(b.width, b.height);
    GTKTTK_SET_FOCUS(state);
#ifdef GTKTTK_GTK_VERSION_3
    QStyle::SFlags sflags =GtkTtk_StateTableLookup(treeview_field_statemap, state);
    ...
    wc->GtkTtk_Style->drawPrimitive(QStyle::PE_PanelLineEdit, &painter,
          QRect(0, 0, b.width, b.height), qApp->palette().active(), sflags,
          QStyleOption(1,1));
#endif /* GTKTTK_GTK_VERSION_3 */
#ifdef GTKTTK_GTK_VERSION_4
    ...
    option.lineWidth = 1;
    option.state |= 
      (QStyle::StateFlag) GtkTtk_StateTableLookup(treeview_field_statemap, state);
    wc->GtkTtk_Style->drawPrimitive(QStyle::PE_FrameGroupBox, &option,
                                    &painter);
#endif /* GTKTTK_GTK_VERSION_4 */
    GTKTTK_CLEAR_FOCUS(state);
    ...
    Drawable d, Ttk_Box b, unsigned state)
{
    ...
    QStyle::SFlags sflags =
                   GtkTtk_StateTableLookup(treeview_client_statemap, state);
    sflags |= QStyle::Style_Horizontal;
    wc->GtkTtk_Style->drawPrimitive(QStyle::PE_GroupBoxFrame, &painter,
          QRect(0, 0, b.width, b.height), qApp->palette().active(), sflags,
          QStyleOption(/*lineWidth*/1, /*midLineWidth*/0,
    ...
    Drawable d, Ttk_Box b, unsigned state)
{
...
#ifdef GTKTTK_GTK_VERSION_3
    QStyle::SFlags sflags = GtkTtk_StateTableLookup(treeheading_border_statemap,
                                                 state);
    sflags |= QStyle::Style_Horizontal;
    wc->GtkTtk_Style->drawPrimitive(QStyle::PE_HeaderSection, &painter,
          QRect(0, 0, b.width, b.height), qApp->palette().active(), sflags);
#endif /* GTKTTK_GTK_VERSION_3 */
#ifdef GTKTTK_GTK_VERSION_4
    ...
    option.rect = QRect(0, 0, b.width, b.height);
    option.state |= 
      (QStyle::StateFlag) GtkTtk_StateTableLookup(treeview_client_statemap, state);
    wc->GtkTtk_Style->drawControl(QStyle::CE_HeaderSection, &option,
                                  &painter);
#endif /* GTKTTK_GTK_VERSION_4 */
    // printf("x=%d, y=%d, w=%d, h=%d\n", b.x, b.y, b.width, b.height);
    GtkTtk_CopyGtkPixmapOnToDrawable(gdkDrawable, d, tkwin,
...
int GtkTtk_Init_TreeView(Tcl_Interp *interp,
